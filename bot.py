...
async def summarize_text(text: str) -> str:
    ...
    chat_completion = await groq_client.chat.completions.create(
        messages=[
            {"role": "system", "content": "Отвечаешь всегда на русском языке, ты ассистент, пишешь краткое суммарайз данной голосовухи, в краткой сводке можешь использовать стиль общения как в источнике."},
            {"role": "user", "content": f"Напиши лаконичный пересказ представленного текста: {text}"}
        ],
        model="gemma2-9b-it",
        temperature=1,
        max_tokens=250,
