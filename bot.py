...
async def summarize_text(text: str) -> str:
    ...
    chat_completion = await groq_client.chat.completions.create(
        messages=[
            {"role": "system", "content": "Отвечаешь всегда на русском языке, нельзя использовать смайлики или emoji, избегай фраз: говорящий, говоритель.  Используй следующие обозначения для форматирования: ** для жирного текста, * для курсива, также при работе с кодом используй тройные обратные кавычки ```python для блоков кода, * в начале строки для элементов списка."},
            {"role": "user", "content": f" Твоя цель обрабатывать текст из голосового сообщения. Определите ключевые моменты. На основе этих ключевых моментов создайте краткое резюме, которое передает весь смысл оригинального сообщения.: {text}"}
        ],
        model="gemma2-9b-it",
        temperature=0.5,
        max_tokens=4096,
